...
import { extractActions } from "@/lib/assistant/navigationUtils";
...
function parseInline(text) {
  ...
  while ((m = regex.exec(text)) !== null) {
    ...
    if (m[2] !== undefined) {
      parts.push(
        <strong key={m.index} style={{ fontWeight: 700, color: "#000" }}>
          {m[2]}
        </strong>,
      );
    } else if (m[3] !== undefined) {
      parts.push(
        <em key={m.index} style={{ fontStyle: "italic", color: T.textMuted }}>
          {m[3]}
        </em>,
      );
...
function RenderCallout({ type, content }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: "13.5px", lineHeight: "1.5", color: T.text }}>
        {parseInline(content)}
      </div>
    </div>
  );
...
function parseBlocks(raw) {
  ...
  while (i < lines.length) {
    ...
    const calloutMatch = line.match(
      /^>\s*!(warning|info|success|danger)\s+(.*)/i,
    );
    if (calloutMatch) {
      blocks.push({
        type: "callout",
...
export default function InaraResponse({
  content,
  onSuggestionsFound,
  onNavigate,
}) {
  const { filteredContent, suggestions, navData } = useMemo(() => {
    if (!content)
      return { filteredContent: "", suggestions: [], navData: null };
    ...
    if (suggestionMatch) {
      ...
      text = text.replace(suggestionMatch[0], "");
    }

    // Use centralized extraction for actions
    const { cleanText, actions } = extractActions(text);
    const navData = actions.find(a => a.action === 'navigate') || null;

    return { filteredContent: cleanText, suggestions, navData };
  }, [content]);

  const blocks = useMemo(() => parseBlocks(filteredContent), [filteredContent]);

  useEffect(() => {
    if (suggestions.length > 0 && onSuggestionsFound) {
      onSuggestionsFound(suggestions);
    }
  }, [suggestions, onSuggestionsFound]);

  if (!blocks.length && !navData) return null;
  ...
  return (
    <div className="inara-rich-response">
      {blocks.map((block, idx) => {
        switch (block.type) {
          case "h1":
            return <RenderH1 key={idx} content={block.content} />;
          case "h2":
            return <RenderH2 key={idx} content={block.content} />;
          case "h3":
            return <RenderH3 key={idx} content={block.content} />;
          case "p":
            return <RenderParagraph key={idx} content={block.content} />;
          case "ul":
            return <RenderUL key={idx} items={block.items} />;
          case "ol":
            return <RenderOL key={idx} items={block.items} />;
          case "code":
            return (
              <RenderCode key={idx} lang={block.lang} content={block.content} />
            );
          case "callout":
            return (
              <RenderCallout
          ...
          case "divider":
            return <RenderDivider key={idx} />;
          case "kv":
            return (
              <RenderKV key={idx} label={block.label} value={block.value} />
            );
          default:
            return null;
        }
      ...
      {navData && (
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            ...
            >
              <Compass size={18} style={{ margin: "auto" }} />
            </div>
            <div>
              <p
                style={{
              ...
              <p style={{ fontSize: "11px", color: T.textMuted, margin: 0 }}>
                ...
                <strong>{navData.label || "this section"}</strong>?
              </p>
            </div>
          </div>
          <button
            onClick={() => onNavigate && onNavigate(navData)}
            style={{
              width: "100%",
              padding: "10px",
              background: T.navy,
