...
export default function AssistantSidePanel({ isOpen, onClose }) {
  ...
  const handleSend = async (e, textOverride = null) => {
    ...
    setInput("");
    const reply = await sendUserMessage(text, { appLocale: locale });
    ...
    if (reply) {
      try {
        ...
        if (navData && navData.path) {
          setNavPending({ 
              path: navData.path, 
              label: navData.label || "Requested Module"
          });
        }
      } catch (err) {
  ...
  return (
    <>
      <style jsx global>{`
        ...
        .dark .inara-tour-popover {
          background: #081422 !important;
          color: white !important;
        }
        .driver-popover-title {
          color: #ff782d !important;
          font-family: 'Outfit', 'Metropolis', sans-serif !important;
        ...
        .inara-click-ripple {
          ...
          width: 40px;
          height: 40px;
          background: rgba(255, 120, 45, 0.4);
          border-radius: 50%;
          transform: translate(-50%, -50%) scale(0);
          animation: inara-ripple 0.8s ease-out;
          pointer-events: none;
          z-index: 9999;
        }
        @keyframes inara-ripple {
      ...
      {!isOpen && (
        <motion.button
          ...
          className="fixed bottom-6 right-6 z-[100] w-12 h-12 bg-gradient-to-br from-[#ff782d] to-[#ea580c] text-white rounded-xl shadow-xl shadow-orange-500/20 flex items-center justify-center group border-2 border-white/10"
        >
      ...
      <motion.div
        animate={{ 
            ...
            scale: cursorPos.opacity ? 1 : 0.5
        }}
        transition={{ type: "spring", damping: 30, stiffness: 200 }}
        className="fixed top-0 left-0 z-[10000] pointer-events-none text-[#ff782d]"
      >
            ...
            >
              ...
              <div className="p-5 border-b border-slate-200 flex items-center justify-between bg-[#081422] text-white relative overflow-hidden">
                ...
                <div className="flex items-center gap-4 relative z-10">
                  <div className="p-2.5 bg-[#ff782d] rounded-2xl shadow-lg shadow-[#ff782d]/20">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1.5">
                      ...
                      <p className="text-[10px] uppercase font-bold tracking-widest text-slate-400">Interactive Guide</p>
                    </div>
                  </div>
                </div>
                <div className="flex items-center gap-1 relative z-10">
                  <button onClick={clearMessages} className="p-2 hover:bg-white/5 rounded-xl transition-all active:scale-90" title="Clear Chat">
              ...
              <div ref={scrollRef} className="flex-1 overflow-y-auto p-5 space-y-8 scroll-smooth bg-white">
                {messages.length === 0 && (
                  <div className="h-full flex flex-col items-center justify-center text-center px-4">
                    <div className="w-16 h-16 bg-slate-50 rounded-2xl flex items-center justify-center mb-6 border border-slate-100">
                      <MessageSquare size={32} className="text-[#081422]" />
                    </div>
                    ...
                    <p className="text-slate-500 text-sm leading-relaxed max-w-[280px] mb-8">
                      I'm Inara, your Invexis guide. Click a shortcut below to start a practice tour.
                    </p>
                    ...
                    <div className="grid grid-cols-2 gap-3 w-full max-w-sm">
                      {quickQuestions.map((q, i) => (
                        <button
                          ...
                          className="flex flex-col items-center gap-2 p-4 bg-slate-50 border border-slate-100 rounded-2xl hover:border-orange-300 hover:bg-orange-50/50 transition-all group"
                        >
                ...
                {messages.map((msg, idx) => (
                  ...
                  >
                    <div className={`max-w-[88%] p-4 rounded-3xl shadow-sm relative transition-all duration-200 ${msg.role === "user" ? "bg-[#081422] text-white rounded-tr-none hover:shadow-md" : "bg-slate-100 border border-slate-200 text-slate-800 rounded-tl-none hover:bg-slate-200/50"}`}>
                      {msg.role === "assistant" && (
                    ...
                    <div className={`flex items-center gap-3 mt-2 px-2 transition-all duration-300 opacity-0 group-hover:opacity-100 ${msg.role === "user" ? "flex-row-reverse" : "flex-row"}`}>
                      <div className="flex items-center gap-1.5">
                        <button onClick={() => handleCopy(msg.content, idx)} className="p-1.5 hover:bg-slate-100 rounded-lg text-slate-400 hover:text-[#ff782d] transition-colors">{copiedId === idx ? <Check size={14} className="text-emerald-500" /> : <Copy size={14} />}</button>
                        {msg.role === 'assistant' && <button onClick={() => handleRefresh(msg)} className="p-1.5 hover:bg-slate-100 rounded-lg text-slate-400 hover:text-[#ff782d] transition-colors"><RotateCcw size={14} /></button>}
                        {msg.role === 'user' && <button onClick={() => handleEdit(msg)} className="p-1.5 hover:bg-slate-100 rounded-lg text-slate-400 hover:text-[#ff782d] transition-colors"><Edit2 size={14} /></button>}
                      </div>
                  ...
                  <div className="flex justify-start">
                    <div className="bg-slate-100 border border-slate-200 p-4 rounded-3xl rounded-tl-none shadow-sm flex items-center gap-2">
                      <div className="flex gap-1.5">
                        {[0, 1, 2].map((i) => (
                          <motion.div key={i} animate={{ y: [0, -6, 0], opacity: [0.4, 1, 0.4] }} transition={{ repeat: Infinity, duration: 0.8, delay: i * 0.15 }} className="w-2 h-2 bg-[#ff782d] rounded-full" />
                        ))}
                      </div>
                    </div>
                  </div>
                ...
                <AnimatePresence>
                  {navPending && !isNavigating && (
                    <motion.div 
                      ...
                      className="p-5 bg-slate-50 border border-slate-200 rounded-[2rem] shadow-xl space-y-4"
                    >
                      <div className="flex items-center gap-3">
                        <div className="p-3 bg-[#081422] rounded-2xl text-white shadow-lg">
                          <Compass size={24} className="text-[#ff782d]" />
                        </div>
                        <div>
              ...
              <div className="p-5 border-t border-slate-100 bg-white">
                <form onSubmit={handleSend} className="relative flex items-center gap-3">
                  <div className="relative flex-1">
                      <input type="text" value={input} onChange={(e) => setInput(e.target.value)} placeholder="Show me how to..." className="w-full pl-5 pr-12 py-4 bg-slate-50 border border-transparent rounded-2xl text-[14px] font-medium focus:outline-none focus:ring-2 focus:ring-[#ff782d]/20 focus:border-[#ff782d] focus:bg-white transition-all shadow-inner" />
                  </div>
                  <button type="submit" disabled={!input.trim() || loading} className={`p-4 rounded-2xl transition-all shadow-lg ${input.trim() && !loading ? "bg-[#081422] text-white shadow-[#081422]/20 scale-100 hover:bg-black active:scale-90" : "bg-slate-100 text-slate-300 scale-95 opacity-50 cursor-not-allowed shadow-none"}`}>
                    <Send size={20} />
                  </button>
                </form>
                <div className="flex items-center justify-between mt-5 px-1">
                    <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest">Inara Smart Tour <span className="text-[#ff782d] ml-1">v4.1</span></p>
                </div>
              </div>
            </motion.div>
          </>
